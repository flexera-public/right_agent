...
module RightScale
  ...
  class WasabiReplicatorSink
    ...
    on_exception { |meth, deliverable, e| ExceptionMailer.deliver_notification(meth, deliverable, e) }
    ...
    #
    # === Return
    # res(OperationResult):: success or error with message
    ...
    #
    # === Return
    # res(OperationResult):: success or error
    ...
    def synchronize_replica_range(options)
      ...
      success = query("wasabi_replicator_sink/synchronize_replica_range for #{replica_class.name}", nil, :email_errors => true) do
        ...
        records.each do |h|
          h = SerializationHelper.symbolize_keys(h)
